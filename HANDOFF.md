...
- 최신 Sites 공개 배포: **버전 29**, 2026-09-13 완료
- Sites 소스 `origin/main`, GitHub `github/main`, 현재 로컬 HEAD: `00dcf9ebf0ccd4c1a0a9bfd7162a714e22285268` (`Add batch PDF processing`)
- GitHub 공개 저장소와 Sites 소스 원격은 위 커밋까지 동기화했다. 상단의 공개 소스 링크가 가리키는 과거 커밋 `cc30b87` 역시 GitHub 이력에 포함되어 404가 나지 않는다.

### 최근 반영 내용

- 일반 노트북 화면(1366×768, 1280×700 등 높이 700px 내외 뷰포트)에서 스크롤 없이 하단 '파일 선택' 버튼과 메인 화면 전체가 한눈에 들어오도록 좌측 메인 카드의 고정 최소 높이(`min-h-[610px]`)를 해제하고, 상하 여백, 드롭존 패딩(`py-14` → `py-6 sm:py-8`), 아이콘 크기 및 우측 카드 간격을 컴팩트하게 최적화했다.
- 대입전형자료 업로드 시 1쪽 학적사항의 입학교명이 탐지는 되지만 다운로드 PDF에서 제거되지 않던 결함을 수정했다. 하단 학교 직인 등 동일 명칭·좌표의 단어가 나중에 중복 레이어로 순회될 때 `pushRectCandidate`가 중복으로 조기 반환하면서 `candidates.at(-1)`(직전에 등록된 학적사항 입학교명 후보)의 `targetGlyphIds`와 `targetQuads`를 하단 직인 글리프로 덮어쓰던 원인을 해결했다. 신규 후보 추가 시에만 타깃을 대입하고, 중복 후보는 기존 일치 후보에 글리프를 병합하도록 보정했다.
- 대입전형자료 1쪽 인적·학적사항 테이블(`학교코드`, `학과(코드)`, `반`, `번호`)에서 학교코드(`school-code`, 예: `J100000673`), 반, 번호 자동 탐지 및 UI 배지를 지원한다.
...
- PDF 2~10개를 선택하면 일괄처리 화면으로 전환한다. 배치 전체에서 `파일별 검토` 또는 `자동 처리`를 선택할 수 있으며, 단일 PDF의 기존 검토·개별 저장 UX는 유지한다.
- 일괄처리는 브라우저 메모리에서 한 파일씩 실행한다. 암호 PDF의 재시도·건너뛰기, 오류 후 다음 파일 계속 처리, 중지·재개, 검토 필요 파일의 수동 재처리를 지원한다. 성공 파일은 `가림PDF_YYYYMMDD-HHmm.zip`으로 저장하고 `처리결과.txt`에 성공·검토 필요·오류·건너뜀 결과를 남긴다.
- `fflate`를 추가해 PDF 항목은 재압축하지 않는 ZIP 스트림으로 묶으며, 중복 결과명에는 `_2`, `_3` 접미사를 붙인다. 배치 상태·ZIP 도우미 단위 테스트는 `lib/batch-processing.test.ts`에 있다.

### 검증 및 다음 작업 시 주의

- 입학교명 중복 글리프 보존 단위 테스트 및 대입전형자료 학교코드 테스트를 포함해 `npm test` 72개 통과·2개 생략, `npx tsc --noEmit`, `npm run lint`, `npm run build`가 모두 통과했다. 원본 `sample/3.pdf`로 1쪽 학적사항 입학교명이 완전히 제거됨을 재현·검증했다.
- 현재 작업 트리는 배포 후 깨끗한 상태여야 한다. 시작 시 먼저 git status --short를 확인하고, 사용자가 만든 변경은 보존한다.
- 공개 Sites를 다시 갱신할 때는 아래 **Sites 공식 배포 방법**을 따른다. 사용자가 명시적으로 커밋·푸시·공개 배포를 요청하기 전에는 외부 반영을 하지 않는다.

- 이 프로젝트는 사용자의 원본 PDF·결과물·개인정보를 Git, Sites 배포물, 외부 로그에 절대 포함하지 않는다. sample/, tmp/는 Git 제외 경로다.

### Sites 공식 배포 방법

공식 Sites는 **ChatGPT에서 사이트 버전을 먼저 저장한 뒤, 그 저장된 버전을 배포**하는 방식이다. 단순 Git 푸시만으로는 공개본이 갱신되지 않는다.

1. `D:\AI_club\RedactPDF`에서 `git status --short`와 `.openai/hosting.json`의 `project_id`를 확인한다. 사용자가 만든 변경과 Git 제외 PDF·결과물은 보존한다.
2. 변경 범위에 맞는 테스트·타입 검사·린트·빌드를 마친 뒤, 개인정보가 없는 소스만 커밋한다. 원본 PDF, 결과 PDF, 렌더 이미지, 진단 로그, 비밀번호는 절대 스테이징하지 않는다.
3. Sites 커넥터의 `create_source_repository_write_credential`로 해당 `project_id`의 **단기 쓰기 자격 증명**을 발급받는다. 토큰은 명령마다 HTTP 인증 헤더로만 사용하며, Git 원격 URL·Git 설정·파일·로그·인수인계 문서·사용자 메시지에 저장하거나 표시하지 않는다.
4. 발급 응답의 브랜치(현재 `main`)로 Sites 원격 `origin`에 정확한 커밋을 푸시한다. 푸시 성공 후 `git rev-parse --verify HEAD`의 전체 SHA를 그대로 사용한다. GitHub 공개 원격도 갱신할 필요가 있으면, 먼저 개인정보 검토와 원격 차이 확인을 마친 후 별도로 푸시한다.
5. 공식 Sites 패키저로 그 동일 SHA의 빌드 산출물만 임시 아카이브에 만든다. 프로젝트 소스 트리나 사용자 PDF를 아카이브로 올리지 않는다.
6. ChatGPT Sites에서 `save_site_version`으로 **푸시된 SHA와 아카이브가 일치하는 버전**을 저장한 다음, 현재 사이트의 접근 범위를 확인한다. 이 사이트는 현재 공개 상태이므로, 사용자가 공개 배포를 요청한 경우에만 `deploy_site_version`으로 그 저장 버전을 배포한다.
7. 배포 상태가 성공인지 확인하고 공개 URL을 연다. 단기 자격 증명과 임시 아카이브는 재사용·공유하지 않으며, 임시 산출물은 Git 제외 경로에만 둔다.

### 다음 버그 수정 세션 시작 프롬프트

```text
가림PDF 프로젝트(D:\AI_club\RedactPDF)의 버그 수정을 이어서 작업해 주세요. 먼저 HANDOFF.md 최상단 절과 git status --short를 확인하고, 사용자가 만든 미커밋 변경은 보존하세요. 현재 공개본은 Sites 버전 29이며 코드 기준 커밋은 00dcf9ebf0ccd4c1a0a9bfd7162a714e22285268입니다.

이번 작업에서는 사용자가 요청한 PDF 탐지·검토·비식별화 버그만 조사하고 수정하세요. 특히 정부24 표에서 개인정보 후보 영역이 과도하게 넓어지는 문제를 다룰 경우, 원본 PDF로 먼저 재현하여 원인을 후보 규칙·글리프 좌표·삭제 경로 중 하나로 확정한 뒤 그 경로만 최소 수정하세요. 추측성 범용 규칙이나 다른 탐지 범위 확장은 추가하지 마세요.

원본 PDF, 결과 PDF, 비밀번호, 렌더 이미지, 진단 로그는 Git·배포물·외부 로그에 절대 포함하지 말고 sample/ 또는 tmp/의 Git 제외 경로에서만 처리하세요. 수정 후에는 관련 단위 테스트와 npm test, npx tsc --noEmit, npm run lint, npm run build를 실행하세요. 커밋·GitHub 푸시·Sites 버전 저장·공개 배포는 사용자가 명시적으로 요청할 때만 진행하며, 요청 시 HANDOFF.md의 Sites 공식 배포 방법을 따르세요.
```
