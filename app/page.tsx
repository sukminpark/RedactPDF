...
const kindLabels: Record<RedactionKind, string> = {
  ...
  address: '학생 주소',
  photo: '학생 사진',
  'government-name': '인적사항 이름',
  'government-staff': '담당자 이름',
  'official-school-name': '학교장 학교명',
  'school-seal': '학교장 직인',
  'issuance-info': '발급정보',
  'school-name': '학교명',
  class: '학급(반)',
...
const kindStyles: Record<RedactionKind, string> = {
  ...
  'government-name': 'border-violet-200 bg-violet-50 text-violet-700',
  'government-staff': 'border-sky-200 bg-sky-50 text-sky-700',
  'official-school-name': 'border-emerald-200 bg-emerald-50 text-emerald-700',
  'school-seal': 'border-red-200 bg-red-50 text-red-700',
...
export default function Home() {
  ...
  const processFile = useCallback(
    async (nextFile: File, password?: string) => {
      await analysisTaskRef.current?.cancel();
      ...
      try {
        ...
        } else if (processingError instanceof PdfProcessingError && (processingError.code === 'password-required' || processingError.code === 'incorrect-password')) {
          passwordRef.current = undefined;
          pendingPasswordFileRef.current = nextFile;
          setStage('idle');
          setError(null);
          setPasswordInput('');
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              이 문서는 암호로 보호되어 있습니다. 비밀번호는 현재 문서를 처리하는 동안에만 메모리에 보관됩니다.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 py-2">
            <Input
              ...
              value={passwordInput}
              aria-label="PDF 비밀번호"
              aria-invalid={passwordPrompt === 'incorrect'}
              placeholder="PDF 비밀번호"
              onChange={(event) => setPasswordInput(event.target.value)}
              onKeyDown={(event) => {
                if (event.key === 'Enter') retryWithPassword();
              }}
            />
            {passwordPrompt === 'incorrect' && (
              <p role="alert" className="text-sm text-red-700">비밀번호가 올바르지 않습니다. 다시 입력해 주세요.</p>
            )}
          </div>
          <DialogFooter>
