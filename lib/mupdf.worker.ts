...
self.onmessage = async (event: MessageEvent<MuPdfRequest>) => {
  ...
  try {
    ...
    const { extractNativePages, redactPdf, redactionValidationInputs, validateRedactedPdf } = await loadEngine();
    ...
    const inputs = redactionValidationInputs(request.pages);
    const validation = validateRedactedPdf(bytes, inputs.expectedPages, inputs.forbidden);
    ...
    if (error instanceof Error && error.message === 'CANCELLED') respond({ id: request.id, type: 'cancelled' });
      ...
      id: request.id,
      type: 'error',
      message: error instanceof Error ? error.message : 'MuPDF 처리 중 오류가 발생했습니다.',
      code: error instanceof PdfProcessingError ? error.code : undefined,
    });
  } finally {
