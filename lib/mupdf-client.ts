...
import { PdfProcessingError, type NativePageText, type MuPdfRequest, type MuPdfResponse, type ValidationResult, type WorkerReviewPage } from './mupdf-types';
...
export class MuPdfWorkerClient {
  ...
  constructor() {
    this.worker.onmessage = (event: MessageEvent<MuPdfResponse>) => {
      ...
      if (message.type === 'error') pending.reject(message.code ? new PdfProcessingError(message.code, message.message) : new Error(message.message));
  ...
  extract(bytes: ArrayBuffer, password?: string, onProgress?: PendingRequest<NativePageText[]>['onProgress']): Promise<NativePageText[]> {
    const copy = bytes.slice(0);
    return this.request({ id: crypto.randomUUID(), type: 'extract', bytes: copy, password }, [copy], onProgress);
  }

  redact(bytes: ArrayBuffer, pages: WorkerReviewPage[], password?: string, onProgress?: PendingRequest<unknown>['onProgress']): Promise<{ bytes: ArrayBuffer; validation: ValidationResult }> {
    const copy = bytes.slice(0);
    return this.request({ id: crypto.randomUUID(), type: 'redact', bytes: copy, pages, password }, [copy], onProgress);
  }
